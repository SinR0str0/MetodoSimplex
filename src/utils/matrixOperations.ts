...
export interface Matrix {
  ...
  export function calculateDeterminant(matrix: Matrix): number {
    ...
    const mat = matrix.data.map(row => [...row]);
    let det = 1;
    ...
    for (let i = 0; i < n; i++) {
      ...
      if (Math.abs(mat[maxRow][i]) < 1e-10) {
        return 0;
      }
  ...
  export function invertMatrixByProduct(B: Matrix, elementaryMatrices: Matrix[]): Matrix {
    let Binv = getIdentityMatrix(B.rows);
